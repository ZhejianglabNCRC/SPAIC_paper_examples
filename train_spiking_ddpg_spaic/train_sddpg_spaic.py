...
from sddpg_agent_spaic import AgentSpiking_spaic
...
def train_sddpg(run_name="SNN_R1", exp_name="Rand_R1", episode_num=(100, 200, 300, 400),
    ...
    train_time='_1'
    # writer
    f_loss = open('/home/dell/workspace_zhang/spiking-ddpg-mapless-navigation/writer/loss'+train_time+'.csv', 'w', encoding='utf-8')
    loss_writer = csv.writer(f_loss)
    loss_writer.writerow(["overall_steps", "actor_loss", "critic_loss"])

    f_time = open('/home/dell/workspace_zhang/spiking-ddpg-mapless-navigation/writer/time'+train_time+'.csv', 'w', encoding='utf-8')
    time_writer = csv.writer(f_time)
    time_writer.writerow(["overall_steps", "ita_time", "action_epsilon","raw_left_wheel_output","raw_right_wheel_output"])

    f_reward = open('/home/dell/workspace_zhang/spiking-ddpg-mapless-navigation/writer/reward'+train_time+'.csv', 'w', encoding='utf-8')
    reward_writer = csv.writer(f_reward)
    reward_writer.writerow(["overall_steps", "avg_reward"])
    ...
    overall_list = pickle.load(open("/home/dell/workspace_zhang/spiking-ddpg-mapless-navigation/training/random_positions/" + exp_name + ".p", "rb"))
    ...
    agent = AgentSpiking_spaic(state_num, action_num, spike_state_num,
    ...
    env = GazeboEnvironment(laser_scan_half_num=laser_half_num, laser_scan_min_dis=laser_min_dis,
                            ...
                            goal_near_th=goal_th, obs_near_th=obs_th,step_time=0.1)
    ...
    while True:
        ...
        for ita in range(ita_per_episode):
            ...
            if len(agent.memory) > batch_size:
                ...
                loss_writer.writerow([overall_steps, actor_loss_value,  critic_loss_value])
            ...
            time_writer.writerow([overall_steps,
                                  ita_time_end - ita_time_start,
                                  agent.epsilon,
                                  raw_snn_action[0],
                                  raw_snn_action[1]])
            ...
            if overall_steps % save_steps == 0:
                agent.save_spaic(overall_steps // save_steps)

            # reward_writer.writerow([overall_steps, episode_reward / (ita + 1)])
            # If Done then break
        ...
        if overall_episode == 999:
            agent.save_spaic('save_sddpg_weights')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--step', type=int, default=5)#default=5
    ...
    # train_sddpg(use_cuda=USE_CUDA, batch_window=0.5)
