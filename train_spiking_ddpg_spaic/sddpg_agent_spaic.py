...
import sys
sys.path.append('../../')
from ddpg_networks import CriticNet
# from training.train_spiking_ddpg.sddpg_networks import ActorNetSpiking
from sddpg_networks_spaic import ActorNetSpiking_spaic


class AgentSpiking_spaic:
    """
    ...
    def __init__(self,
        ...
        if self.use_cuda:
            ...
            self.device_tp="cuda" if torch.cuda.is_available() else "cpu"
        else:
        ...
        """
        # using spaic net replace the ordinal network
        self.actor_net = ActorNetSpiking_spaic(self.spike_state_num, self.action_num, self.device,
                                         hidden1=actor_net_dim[0],
                                         hidden2=actor_net_dim[1],
                                         hidden3=actor_net_dim[2],
                                         batch_window=self.batch_window)
        self.target_actor_net = ActorNetSpiking_spaic(self.spike_state_num, self.action_num, self.device,
                                                        hidden1=actor_net_dim[0],
                                                        hidden2=actor_net_dim[1],
                                                        hidden3=actor_net_dim[2],
                                                        batch_window=self.batch_window)
        ...
        self.actor_net.set_simulator('torch', self.device_tp)
        self.target_actor_net.set_simulator('torch', self.device_tp)

        self.actor_net._simulator.set_runtime(self.batch_window)
        self.target_actor_net._simulator.set_runtime(self.batch_window)
        if self.actor_net._simulator.builded is False:
            self.actor_net.build()
        if self.target_actor_net._simulator.builded is False:
            self.target_actor_net.build()

        # self.actor_net.state_from_dict('actornet_init')
        # self.actor_net.save_state('actornet_read')
        self._hard_update_spaic(self.target_actor_net, self.actor_net)
    ...
    def act(self, state, explore=True, train=True):
        ...
        with torch.no_grad():
            ...
            state_spikes = np.array(state).reshape((1, -1))

            self.actor_net.input(state_spikes)
            self.actor_net.run(self.batch_window)
            action = (self.actor_net.output.predict/(self.batch_window*10)).to('cpu')
            self.pcnt +=1
            if self.pcnt>0 and self.pcnt%64==0:
                print('out put count:',action)
                self.pcnt=0
    ...
    def replay(self):
        ...
        with torch.no_grad():
            ...
            self.actor_net.input(nstate_spikes_batch)
            self.actor_net.run(self.batch_window)
            naction_batch = self.actor_net.output.predict/(self.batch_window*10)
        ...
        self.actor_net.run(self.batch_window)
        current_action = self.actor_net.output.predict/(self.batch_window*10 )

        actor_loss = -self.critic_net([state_batch, current_action])
        actor_loss = actor_loss.mean()
        if self.pcnt > 0 and self.pcnt % 50 == 0:
        ...
        if self.step_ita % self.target_update_steps == 0:
            ...
            self._soft_update_spaic(self.target_actor_net, self.actor_net)
    ...
    def save_spaic(self, episode):
        self.actor_net.save_state("sddpg_{}".format(episode))
        return
    ...
    def _random_minibatch(self):
        ...
        # state_spikes_batch = []
        # for i in range(spike_state_value_batch.shape[0]):
        #     state_spikes_batch.append(spike_state_value_batch[i, :].reshape((1, -1)))
        # nstate_spikes_batch = []
        # for i in range(spike_nstate_value_batch.shape[0]):
        #     nstate_spikes_batch.append(spike_state_value_batch[i, :])
    ...
    def _hard_update_spaic(self, target, source,env_name='actornet'):
        source.save_state(env_name)
        target.state_from_dict(env_name)

    def _soft_update_spaic(self, target, source,env_name='actornet'):
        source.save_state(env_name)
        target.state_from_dict(env_name,ratio=self.target_tau)

if __name__ == '__main__':
    actnet=AgentSpiking_spaic(state_num=22,action_num=2,spike_state_num=24)
